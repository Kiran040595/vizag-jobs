  ...
  const filteredJobs = useMemo(
    () =>
      sortJobsForListing(
        allJobs.filter(
          (job) =>
            isPublicFresherListingJob(job) && jobMatchesSearchText(job, searchTerm),
        ),
      ),
    [allJobs, searchTerm],
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-blue-50/30 to-white">
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-3 py-5 pb-mobile-chrome sm:gap-8 sm:px-6 sm:py-10 lg:px-8">
        <HeroSection
          title="Fresher Jobs in Vizag"
          subtitle="Entry-level job opportunities for fresh graduates in Visakhapatnam"
