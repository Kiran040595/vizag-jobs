...
export default function EmployerGoogleButton({
  ...
  const handleClick = async () => {
    ...
    try {
      await signInWithGoogle({ companyName: companyName?.trim() || undefined });
    } catch (oauthError) {
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        disabled={isLoading}
        onClick={handleClick}
        className="flex h-12 w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white text-sm font-semibold text-slate-800 transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-70"
      >
