...
function AdminAccessDenied() {
  const { authError, signOut, user } = useAdminAuth();
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.18),_transparent_35%),linear-gradient(180deg,_#eff6ff_0%,_#ffffff_45%,_#f8fafc_100%)] px-4 py-12">
      <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70">
        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-rose-500">Admin access required</p>
        <h1 className="mt-3 text-3xl font-black text-slate-950">This account is not allowlisted.</h1>
        <p className="mt-4 text-sm leading-6 text-slate-600">
          You are signed in as <span className="font-semibold text-slate-900">{user?.email}</span>, but this
          account does not exist in `public.admin_users`.
        </p>
        {authError ? (
...
export default function AdminRoute({ children }) {
  const { isLoading, isSupabaseConfigured, isAdmin, session } = useAdminAuth();
      ...
      <div className="min-h-screen bg-slate-950 px-4 py-12 text-white">
        <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/5 p-8">
          ...
          <p className="mt-3 text-sm text-slate-300">
            Add `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` before using the admin panel.
          </p>
        </div>
      </div>
    );
