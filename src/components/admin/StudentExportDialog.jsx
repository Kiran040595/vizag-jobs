...
import {
  ...
  getDefaultStudentExportColumnIds,
...
export default function StudentExportDialog({
  ...
  const [selectedIds, setSelectedIds] = useState(() => getDefaultStudentExportColumnIds());
  ...
  const selectDefaults = () => {
    setError('');
    setSelectedIds(getDefaultStudentExportColumnIds());
  };

  const selectAll = () => {
    setError('');
    setSelectedIds(STUDENT_EXPORT_COLUMNS.map((column) => column.id));
  };
  ...
  const handleDownload = async () => {
    try {
      await downloadStudentExcel(students, selectedIds, scopeLabel);
      onClose?.();
    } catch (downloadError) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-4 sm:items-center">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 id="student-export-title" className="text-xl font-black text-slate-950">
              Download students (Excel)
            </h2>
            <p className="mt-1 text-sm text-slate-600">
              Choose columns, then download {students.length} student
              {students.length === 1 ? '' : 's'} for{' '}
              <span className="font-semibold text-slate-800">{scopeLabel}</span>.
            </p>
          </div>
          <button
            type="button"
